...
@Component({
  selector: 'app-metricas-taller',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './metricas-taller.component.html',
})
export class MetricasTallerComponent {
  desde = '';
  hasta = '';
  loading = false;
  errorMsg = '';
  ...
  cargar(): void {
    ...
    this.reportes.metricasTaller({
      ...
      error: (err) => {
        this.errorMsg = err.error?.detail ?? 'No se pudo cargar el reporte del taller';
        this.loading = false;
      },
    });
  }
  ...
  exportarCSV(): void {
    ...
    this.descargar('reporte_taller.csv', `${header}\n${rows.join('\n')}`, 'text/csv;charset=utf-8;');
  }
  ...
  exportarExcel(): void {
    ...
    this.descargar('reporte_taller.xls', `${header}\n${rows.join('\n')}`, 'application/vnd.ms-excel;charset=utf-8;');
  }
  ...
  exportarPDF(): void {
    ...
    const tableRows = this.resumen.detalle_pagos
      ...
      .join('');
    const html = `<!doctype html><html><body><h3>Reporte del Taller</h3><table border="1" cellspacing="0" cellpadding="6"><tr><th>Pago</th><th>Cotizacion</th><th>Incidente</th><th>Monto</th><th>Metodo</th><th>Fecha</th></tr>${tableRows}</table></body></html>`;
